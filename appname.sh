...
appinfo -d " " -o ands,pn -pn "$system" -3 2>/dev/null | sort | while read name; do
	...
	if [[ $1 = twrp ]]; then
		...
		[[ ! -e $nametxt ]] && echo "#不需要恢復還原的應用請在開頭注釋# 比如#xxxxxxxx 酷安" >"$nametxt"
...
endtime 1
[[ $1 = twrp ]] && echoRgb " 輸出包名結束 請編輯$MODDIR/recovery/script/Apkname.txt確認無法開機時需要備份應用" || echo " 輸出包名結束 請查看$txtpath/Apkname.txt"
exit 0
