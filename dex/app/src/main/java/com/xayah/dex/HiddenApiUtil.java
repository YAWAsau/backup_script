...
public class HiddenApiUtil {
    private static final String HIDDEN_API_UTIL_VERSION = "v13.1";
    ...
    private static void onHelp() {
        ...
        System.out.println();
        System.out.println("  getRuntimePermissions USER_ID PACKAGE PACKAGE PACKAGE ...  取得 Runtime 權限與特殊 AppOps");
        System.out.println();
        System.out.println("  getNotificationSettings USER_ID PACKAGE PACKAGE PACKAGE ...  取得通知總開關、通知分類、圓點、對話相關設定");
        System.out.println();
        System.out.println("  setNotificationSettings USER_ID [PACKAGE KEY VALUE KEY VALUE ...] [PACKAGE KEY VALUE ...]  批量還原通知設定");
        System.out.println();
        System.out.println("  getBatterySettings USER_ID PACKAGE PACKAGE PACKAGE ...  批量取得背景使用/AppOps與Doze白名單");
        System.out.println();
        System.out.println("  setBatterySettings USER_ID [PACKAGE KEY VALUE KEY VALUE ...] [PACKAGE KEY VALUE ...]  批量還原背景使用/AppOps與Doze白名單");
        System.out.println();
        System.out.println("  grantRuntimePermission USER_ID [PACKAGE PERM_NAME PERM_NAME ...] [PACKAGE PERM_NAME ...]  授予權限或允許 android:xxx AppOps");
        System.out.println();
        System.out.println("  revokeRuntimePermission USER_ID [PACKAGE PERM_NAME PERM_NAME ...] [PACKAGE PERM_NAME ...]  撤銷權限或拒絕 android:xxx AppOps");
        System.out.println();
        System.out.println("  setOpsMode USER_ID [PACKAGE OP MODE OP MODE ...] [PACKAGE OP MODE ...]  批量設定 AppOps mode");
        System.out.println();
            ...
            for (int i = 2; i < args.length; i++) {
                ...
                for (String packageName : packageNames) {
                    try {
                        ...
                        System.out.println("失敗，已跳過: " + packageName);
                    }
                }
            }
            System.exit(0);
    ...
    private static void getPackageArchiveInfo(String[] args) {
        try {
            ...
            if (packageInfo != null && packageInfo.applicationInfo != null) {
                ...
                throw new PackageManager.NameNotFoundException("解析 APK 資訊失敗!");
            }
            System.exit(0);
        } catch (Exception e) {
            e.printStackTrace(System.out);
                ...
                for (int i = 0; i < requestedPermissions.length; i++) {
                    try {
                        ...
                        if (op != AppOpsManagerHidden.OP_NONE) {
                            // 對有 AppOps 對應的權限，UI 開關實際上看的是 mode。
                            // 例如 USE_FULL_SCREEN_INTENT 可能 requested flag 還是 granted，
                            // 但 AppOps mode=MODE_IGNORED 時系統設定頁會顯示關閉。
                    ...
                    try {
                        if (isAppOpOnlyToken(permName)) {
                            if (!setAppOpModeIfPossible(pm, appOpsManager, userId, permissionSet.packageName, permName, AppOpsManagerHidden.MODE_ALLOWED)) {
                                throw new IllegalArgumentException("未知 AppOps 權限: " + permName);
                            }
                        } else {
                            try {
                        ...
                        System.out.println("失敗，已跳過: " + permissionSet.packageName + " " + permName);
                    }
                }
            }
            System.exit(0);
                ...
                try {
                    ...
                    for (OpMode opMode : opModeSet.opModes) {
                        try {
                            ...
                            System.out.println("失敗，已跳過: " + opModeSet.packageName + " " + e.getMessage());
                        }
                    }
                } catch (Exception e) {
                    System.out.println("失敗，已跳過: " + opModeSet.packageName + " " + e.getMessage());
        ...
        try {
            ...
            for (String packageName : packageNames) {
                try {
                    ...
                    System.out.println("失敗，已跳過: " + packageName + " " + e.getMessage());
                }
            }
            System.exit(0);
        } catch (Exception e) {
                ...
                try {
                    ...
                    for (NotificationSettingValue item : set.items) {
                        try {
                            ...
                            System.out.println("失敗，已跳過: " + set.packageName + " " + item.key + " " + e.getMessage());
                        }
                    }
                } catch (Exception e) {
                    System.out.println("失敗，已跳過: " + set.packageName + " " + e.getMessage());
                }
            }
            System.exit(0);
        } catch (Exception e) {
    ...
    private static void applyBatterySetting(AppOpsManagerHidden appOpsManager, String packageName, int uid, String key, String value) throws Exception {
        if ("battery_opt".equals(key) || "BATTERY:RUN_ANY_IN_BACKGROUND".equals(key)) {
            ...
            if (op == AppOpsManagerHidden.OP_NONE) throw new IllegalArgumentException("找不到 RUN_ANY_IN_BACKGROUND AppOp");
        ...
        if ("BATTERY:RUN_IN_BACKGROUND".equals(key)) {
            ...
            if (op == AppOpsManagerHidden.OP_NONE) throw new IllegalArgumentException("找不到 RUN_IN_BACKGROUND AppOp");
    ...
    private static Set<String> getDeviceIdleWhitelist() {
        ...
        try {
            ...
            // getFullPowerWhitelist 涵蓋系統+使用者白名單 (比 getSystemPowerWhitelist 更完整)
            ...
            // 走不通就退回 dumpsys 文字解析 (相容性後備, 見下方 getDeviceIdleWhitelistViaDumpsys)
        }
    ...
    private static void setDeviceIdleWhitelist(String packageName, boolean enabled) {
        try {
            ...
            // 走不通就退回 dumpsys/cmd 指令 (相容性後備)
        }
        setDeviceIdleWhitelistViaDumpsys(packageName, enabled);
    }

    // ---- 後備機制: 透過 dumpsys 文字輸出解析/操作 (僅當上方 AIDL 介面呼叫失敗時使用) ----
    ...
    private static void setDeviceIdleWhitelistViaDumpsys(String packageName, boolean enabled) {
        ...
        // 優先走 cmd，成功就停止；只有失敗才 fallback 到 dumpsys，避免兩條 shell 都跑造成變慢。
    ...
    private static Object callRequired(Object target, CallSpec... specs) throws Exception {
        ...
        throw new IllegalStateException(last != null ? last.getMessage() : "找不到符合的方法");
    }
    ...
    private static List<List<String>> parseBracketGroups(String[] args, int start) {
        ...
        for (int i = start; i < args.length; i++) {
            for (String rawToken : args[i].trim().split("\\s+")) {
                ...
                if (startsGroup) {
                    if (current != null) {
                        throw new IllegalArgumentException("分組不可嵌套: " + rawToken);
                    }
                    current = new ArrayList<>();
                    rawToken = rawToken.substring(1);
                }
                if (current == null) {
                    throw new IllegalArgumentException("缺少分組起始符號 [: " + rawToken);
                }
                if (endsGroup) {
        ...
        if (current != null) {
            throw new IllegalArgumentException("缺少分組結束符號 ]");
        }
        return groups;
    }
